...
".container { background: var(--container-bg); border: 2px solid var(--border-color); padding: 3rem 2.5rem; border-radius: 24px; box-shadow: 0 0 30px rgba(139, 92, 246, 0.15), 0 15px 50px rgba(0, 0, 0, 0.8), inset 0 0 15px rgba(139, 92, 246, 0.1); backdrop-filter: blur(20px); width: 480px; text-align: center; position: relative; z-index: 10; transition: all 0.4s cubic-bezier(0.16, 1, 0.3, 1); }"
...
".progress-wrapper { display: none; margin-top: 2rem; text-align: left; background: rgba(6, 6, 12, 0.9); border: 1.5px solid rgba(188, 57, 250, 0.3); border-radius: 14px; padding: 1.2rem; box-shadow: 0 0 20px rgba(188, 57, 250, 0.15), inset 0 2px 4px rgba(0,0,0,0.8); animation: fadeIn 0.3s ease-out forwards; }"
...
".progress-bar { background: rgba(255, 255, 255, 0.03); border-radius: 9999px; height: 10px; overflow: hidden; margin-top: 0.7rem; border: 1px solid rgba(188, 57, 250, 0.2); box-shadow: inset 0 1px 3px rgba(0,0,0,0.8); }"
...
".status-text { font-size: 0.8rem; color: #a78bfa; margin-top: 0.5rem; display: flex; justify-content: space-between; font-weight: 300; }"
"</style>"
"</head>"
"<body>"
"<div class='container'>"
...
"      color: `hsl(${270 + Math.random() * 40}, 100%, 75%)`"
"    });"
"  }"
"});"
"function drawSpark(ctx, cx, cy, spikes, outerRadius, innerRadius, color, alpha) {"
"  let rot = Math.PI / 2 * 3;"
"  let x = cx;"
"  let y = cy;"
...
"    p.alpha -= 0.025;"
"    p.size *= 0.96;"
"    drawSpark(ctx, p.x, p.y, 4, p.size, p.size / 2.5, p.color, p.alpha);"
