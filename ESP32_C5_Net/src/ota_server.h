...
"<head>"
"<meta charset='utf-8'>"
"<title>HackerHMI OTA Neon Dashboard</title>"
...
".container { background: var(--container-bg); border: 2px solid var(--border-color); padding: 3rem 2.8rem; border-radius: 24px; box-shadow: 0 0 30px rgba(139, 92, 246, 0.15), 0 15px 50px rgba(0, 0, 0, 0.8), inset 0 0 15px rgba(139, 92, 246, 0.1); backdrop-filter: blur(20px); width: 100%; max-width: 550px; text-align: center; position: relative; z-index: 10; transition: filter 0.4s ease, transform 0.4s ease; }"
...
".container.blurred { filter: blur(4px) brightness(0.8); pointer-events: none; transform: scale(0.98); }"
"h1 { font-size: 2.2rem; font-weight: 700; margin: 0 0 0.5rem 0; background: linear-gradient(135deg, #f472b6, #c084fc, #6366f1); -webkit-background-clip: text; -webkit-text-fill-color: transparent; letter-spacing: -0.03em; text-shadow: 0 0 20px rgba(188, 57, 250, 0.3); }"
".subtitle { color: var(--text-secondary); font-size: 0.95rem; margin-bottom: 2.5rem; font-weight: 400; text-transform: uppercase; letter-spacing: 0.08em; text-shadow: 0 0 8px rgba(139, 92, 246, 0.4); }"
".option-grid { display: grid; grid-template-columns: 1fr; gap: 1rem; margin-bottom: 2rem; }"
".option-card { background: rgba(6, 6, 12, 0.6); border: 1.5px solid rgba(139, 92, 246, 0.2); border-radius: 14px; padding: 1.2rem; text-align: left; cursor: pointer; transition: all 0.3s cubic-bezier(0.25, 0.8, 0.25, 1); display: flex; align-items: center; gap: 1.2rem; position: relative; }"
".option-card:hover { background: rgba(139, 92, 246, 0.05); border-color: #c084fc; transform: translateY(-3px); box-shadow: 0 0 15px rgba(192, 132, 252, 0.35); }"
".option-card:active { transform: translateY(-1px) scale(0.99); }"
".option-card.active { background: rgba(188, 57, 250, 0.08); border-color: var(--accent-neon); box-shadow: 0 0 20px rgba(188, 57, 250, 0.4), inset 0 0 8px rgba(188, 57, 250, 0.2); }"
".icon-wrapper { background: rgba(139, 92, 246, 0.1); border-radius: 10px; width: 44px; height: 44px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; transition: all 0.3s ease; border: 1px solid rgba(139, 92, 246, 0.2); }"
".option-card:hover .icon-wrapper { transform: rotate(8deg) scale(1.08); background: rgba(188, 57, 250, 0.2); border-color: #c084fc; box-shadow: 0 0 8px rgba(188, 57, 250, 0.4); }"
".option-card.active .icon-wrapper { background: rgba(188, 57, 250, 0.25); border-color: var(--accent-neon); box-shadow: 0 0 10px rgba(188, 57, 250, 0.5); }"
".icon-wrapper svg { width: 22px; height: 22px; stroke: #a78bfa; stroke-width: 1.5; fill: none; transition: all 0.3s ease; }"
".option-card.active .icon-wrapper svg { stroke: #f472b6; filter: drop-shadow(0 0 3px rgba(244, 114, 182, 0.6)); }"
".option-info { display: flex; flex-direction: column; }"
".option-title { font-size: 0.95rem; font-weight: 600; color: var(--text-primary); margin-bottom: 0.2rem; transition: all 0.3s; }"
".option-card:hover .option-title { color: #fff; text-shadow: 0 0 8px rgba(255,255,255,0.6); }"
".option-card.active .option-title { color: #fff; text-shadow: 0 0 10px var(--accent-glow); }"
".option-desc { font-size: 0.8rem; color: #9ca3af; font-weight: 300; }"
".upload-box { border: 2px dashed rgba(188, 57, 250, 0.3); border-radius: 16px; padding: 2.2rem 1rem; margin-bottom: 2rem; cursor: pointer; transition: all 0.3s ease; background: rgba(139, 92, 246, 0.01); display: flex; flex-direction: column; align-items: center; }"
".upload-box:hover { border-color: var(--accent-neon); background: rgba(188, 57, 250, 0.04); box-shadow: 0 0 20px rgba(188, 57, 250, 0.2); }"
".upload-box svg { width: 42px; height: 42px; fill: none; stroke: #a78bfa; stroke-width: 1.5; margin-bottom: 0.8rem; transition: all 0.3s ease; }"
".upload-box:hover svg { stroke: var(--accent-neon); transform: translateY(-3px); filter: drop-shadow(0 0 5px var(--accent-glow)); }"
"#uploadText { font-size: 0.9rem; color: #9ca3af; font-weight: 400; transition: all 0.3s; word-break: break-all; }"
".upload-box:hover #uploadText { color: #fff; text-shadow: 0 0 5px rgba(255,255,255,0.4); }"
".btn { background: linear-gradient(135deg, #c084fc, #8b5cf6, #4f46e5); color: white; border: none; padding: 0.95rem 1.5rem; border-radius: 12px; font-weight: 600; font-size: 0.95rem; font-family: 'Outfit', sans-serif; cursor: pointer; width: 100%; transition: all 0.25s cubic-bezier(0.2, 0.8, 0.2, 1); box-shadow: 0 0 15px rgba(139, 92, 246, 0.4); position: relative; overflow: hidden; border: 1px solid rgba(255, 255, 255, 0.1); }"
".btn::after { content: ''; position: absolute; top: 0; left: -50%; width: 200%; height: 100%; background: linear-gradient(to right, rgba(255,255,255,0) 0%, rgba(255,255,255,0.25) 50%, rgba(255,255,255,0) 100%); transform: skewX(-25deg); transition: 0.75s; }"
".btn:hover::after { left: 125%; }"
".btn:hover { transform: translateY(-2px); box-shadow: 0 0 25px rgba(188, 57, 250, 0.6), 0 5px 15px rgba(139, 92, 246, 0.3); border-color: rgba(255, 255, 255, 0.25); }"
".btn:active { transform: translateY(1px) scale(0.99); box-shadow: 0 0 10px rgba(139, 92, 246, 0.2); }"
".modal-overlay { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(3, 3, 7, 0.55); backdrop-filter: blur(5px); -webkit-backdrop-filter: blur(5px); display: flex; align-items: center; justify-content: center; z-index: 100; opacity: 0; pointer-events: none; transition: opacity 0.4s ease; }"
".modal-overlay.active { opacity: 1; pointer-events: auto; }"
".modal-card { background: rgba(12, 12, 22, 0.95); border: 2px solid var(--accent-neon); border-radius: 24px; padding: 2.2rem 1.8rem; width: 90%; max-width: 480px; box-shadow: 0 0 50px rgba(188, 57, 250, 0.4), 0 20px 60px rgba(0, 0, 0, 0.9); transform: scale(0.85); transition: transform 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275); text-align: left; }"
".modal-overlay.active .modal-card { transform: scale(1); }"
"@media (max-width: 520px) {"
"  .container { padding: 1.8rem 1.2rem; border-radius: 18px; }"
"  h1 { font-size: 1.65rem; }"
"  .subtitle { font-size: 0.8rem; margin-bottom: 1.5rem; }"
"  .option-card { padding: 0.95rem; gap: 0.8rem; }"
...
"  .upload-box { padding: 1.5rem 0.8rem; margin-bottom: 1.5rem; }"
"  .btn { padding: 0.85rem 1.2rem; font-size: 0.9rem; }"
"  .modal-card { padding: 1.5rem 1.2rem; width: 92%; }"
"}"
".progress-title-wrapper { display: flex; justify-content: space-between; align-items: center; font-size: 0.95rem; color: var(--text-primary); font-weight: 600; }"
".progress-bar { background: rgba(255, 255, 255, 0.03); border-radius: 9999px; height: 12px; overflow: hidden; margin-top: 1rem; border: 1px solid rgba(188, 57, 250, 0.3); box-shadow: inset 0 1px 3px rgba(0,0,0,0.8); }"
".progress-fill { background: linear-gradient(90deg, #3b82f6, #a855f7, #f472b6, #3b82f6); background-size: 200% 100%; animation: glow-shift 2s linear infinite; width: 0%; height: 100%; border-radius: 9999px; transition: width 0.15s cubic-bezier(0.4, 0, 0.2, 1); box-shadow: 0 0 15px rgba(188, 57, 250, 0.8); position: relative; }"
".progress-fill::after { content: ''; position: absolute; top: 0; left: 0; bottom: 0; right: 0; background-image: linear-gradient(45deg, rgba(255, 255, 255, 0.2) 25%, transparent 25%, transparent 50%, rgba(255, 255, 255, 0.2) 50%, rgba(255, 255, 255, 0.2) 75%, transparent 75%, transparent); background-size: 15px 15px; z-index: 1; animation: progress-bar-stripes 1s linear infinite; border-radius: 9999px; }"
...
".status-text { font-size: 0.85rem; color: #a78bfa; margin-top: 0.8rem; display: flex; justify-content: space-between; font-weight: 300; }"
"</style>"
"</head>"
"<body>"
"<div class='container'>"
...
"    let alpha = 1 - (i / maxPoints);"
"    ctx.strokeStyle = `rgba(188, 57, 250, ${alpha})`;"
"    ctx.lineWidth = (maxPoints - i) * 0.35;"
"    ctx.stroke();"
"  }"
...
"    if (ext !== 'bin' && ext !== 'icl') {"
"      alert('Chỉ chấp nhận các file định dạng .bin hoặc .icl!');"
"      document.getElementById('fileInput').value = '';"
"      document.getElementById('uploadText').innerText = 'Kéo thả hoặc Click để chọn file .bin / .icl';"
"      return;"
"    }"
"    document.getElementById('uploadText').innerText = file.name + ' (' + (file.size/1024/1024).toFixed(2) + ' MB)';"
"  }"
"}"
"function showModal() {"
...
"  const ext = file.name.split('.').pop().toLowerCase();"
"  if (chip === 'esp32' && ext !== 'bin') {"
"    alert('Lỗi: Nâng cấp firmware ESP32-C5 chỉ chấp nhận tệp tin .bin!');"
"    return;"
"  }"
"  if (chip === 'rp2350' && ext !== 'bin') {"
"    alert('Lỗi: Nâng cấp firmware RP2350 chỉ chấp nhận tệp tin .bin!');"
"    return;"
"  }"
"  if (chip === 'dwin' && ext !== 'bin' && ext !== 'icl') {"
"    alert('Lỗi: Nâng cấp giao diện DWIN chỉ chấp nhận tệp tin .bin hoặc .icl!');"
"    return;"
"  }"
"  showModal();"
...
"  document.getElementById('statusInfo').innerText = 'Đang khởi tạo kết nối...';"
...
"      document.getElementById('statusInfo').innerText = 'Đang nạp: ' + pct + '%';"
"    }"
"  };"
"  xhr.onload = function() {"
"    if(xhr.status === 200) {"
"      document.getElementById('statusInfo').innerText = 'Nâng cấp thành công!';"
"      setTimeout(() => { alert('Nâng cấp thành công!'); hideModal(); }, 400);"
"    } else {"
"      alert('Nâng cấp thất bại: ' + xhr.responseText);"
"      hideModal();"
"    }"
"  };"
"  xhr.onerror = function() { alert('Lỗi kết nối máy chủ OTA!'); hideModal(); };"
